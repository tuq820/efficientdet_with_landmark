...
def get_args():
    ...
    parser.add_argument("--pretrained_model", type=str, default="trained_models/signatrix_efficientdet_majia.pth")
...
def test_img(opt):
    ...
    if torch.cuda.is_available():
        new_image = new_image.cuda()
    with torch.no_grad():
        scores, labels, boxes, landmarks = model(new_image)
        boxes /= scale
        landmarks /= scale
    if boxes.shape[0] == 0:
        print('no box')
    ...
    for box_id in range(boxes.shape[0]):
        ...
        for k in range(4):
            cv2.circle(output_image, (landmarks[box_id][2*k], landmarks[box_id][2*k+1]), 2, (0, 255, 0), 2)
