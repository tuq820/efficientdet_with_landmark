...
from src.utils import BBoxTransform, ClipBoxes, Anchors, RegressionTransform
# from src.loss import FocalLoss   # 没有landmark
from src.loss_box_landmark import FocalLoss   # 增加了landmark
...
class LandmarkRegressor(nn.Module):
    def __init__(self, in_channels, num_anchors, num_layers):
        ...
        self.header = nn.Conv2d(in_channels, num_anchors * 8, kernel_size=3, stride=1, padding=1)

    def forward(self, inputs):
        inputs = self.layers(inputs)
        inputs = self.header(inputs)
        output = inputs.permute(0, 2, 3, 1)
        return output.contiguous().view(output.shape[0], -1, 8)
...
class EfficientDet(nn.Module):
    def __init__(self, num_anchors=9, num_classes=20, compound_coef=0):
        ...
        self.classifier = Classifier(in_channels=self.num_channels, num_anchors=num_anchors, num_classes=num_classes,
                                     num_layers=3 + self.compound_coef // 3)
        self.landmarkregressor = LandmarkRegressor(in_channels=self.num_channels, num_anchors=num_anchors,
                                   num_layers=3 + self.compound_coef // 3)
        ...
        self.landmarkregressor.header.weight.data.fill_(0)
        self.landmarkregressor.header.bias.data.fill_(0)
    ...
    def forward(self, inputs):
        ...
        landmarkregression = torch.cat([self.landmarkregressor(feature) for feature in features], dim=1)
        anchors = self.anchors(img_batch)

        if is_training:
            # return self.focalLoss(classification, regression, anchors, annotations)
            return self.focalLoss(classification, regression, landmarkregression, anchors, annotations)
        else:
            # transformed_anchors = self.regressBoxes(anchors, regression)
            # transformed_anchors = self.clipBoxes(transformed_anchors, img_batch)
            bboxes, landmarks = self.regressBoxes(anchors, regression, landmarkregression, img_batch)

            # return classification, bboxes, landmarks
            ...
            landmarks = landmarks[:, scores_over_thresh, :]
            scores = scores[:, scores_over_thresh, :]

            anchors_nms_idx = nms(torch.cat([bboxes, scores], dim=2)[0, :, :], 0.5)

            nms_scores, nms_class = classification[0, anchors_nms_idx, :].max(dim=1)

            return [nms_scores, nms_class, bboxes[0, anchors_nms_idx, :], landmarks[0, anchors_nms_idx, :]]
