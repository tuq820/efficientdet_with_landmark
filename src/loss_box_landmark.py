...
class FocalLoss(nn.Module):
    ...
    def forward(self, classifications, regressions, landmarkregressions, anchors, annotations):
        alpha = 0.25
        gamma = 2.0
        ...
        for j in range(batch_size):
            ...
            landmarkregression = landmarkregressions[j, :, :]

            annotation = annotations[j, :, :]
            # print('annotation1:', annotation)
            # bbox_annotation = bbox_annotation[bbox_annotation[:, 4] != -1]
            annotation = annotation[annotation[:, 4] != -1]
            bbox_annotation = annotation[:, :5]
            ldm_annotation = annotation[:, 5:]

            # print('classification:', classification)
            # print('classification shape:', classification.shape)
            # print('regression:', regression)
            # print('regression shape:', regression.shape)
            # print('landmarkregression:', landmarkregression)

            # print('annotation:', annotation.cpu().numpy())
            # print('bbox_annotation:', bbox_annotation.cpu().numpy())
            # print('ldm_annotation:', ldm_annotation.cpu().numpy())
            ...
            if bbox_annotation.shape[0] == 0:
                if torch.cuda.is_available():
                    ...
                    ldm_regression_losses.append(torch.tensor(0).float())
            ...
            if ldm_positive_indices.sum() > 0:
                ...
                ldm_targets = torch.stack((l0_x, l0_y, l1_x, l1_y, l2_x, l2_y, l3_x, l3_y))
                ldm_targets = ldm_targets.t()

                # Rescale
                scale = torch.ones(1, 8) * 0.1
                if torch.cuda.is_available():
                    scale = scale.cuda()
                ldm_targets = ldm_targets / scale#.cuda()

                ldm_regression_loss = self.smoothl1(ldm_targets, landmarkregression[ldm_positive_indices, :])
        ...
        return torch.stack(classification_losses).mean(dim=0, keepdim=True), \
               ...
               torch.stack(ldm_regression_losses).mean(dim=0,keepdim=True)
